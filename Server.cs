...
{
    // Моделі даних для користувачів, гаманців та транзакцій
    public class User
    {
    ...
    // DTO для передачі даних при трансфері між гаманцями
    ...
    public class Server
    {
                    ...
                    while (true)
                    {
                        ...
                        switch (packet.Type)
                        {
                            ...
                            case MessageType.TransferRequest:
                                ...
                                await HandleTransferAsync(stream, authenticatedUser.Id, packet.Data);
                                break;
                            default:
        ...
        private static async Task HandleTransferAsync(NetworkStream stream, int userId, string data)
        {
            try
            {
                // Десеріалізуємо дані трансферу з отриманого JSON
                var transfer = JsonSerializer.Deserialize<TransferDto>(data);
                ...
                using (SqlConnection db = new SqlConnection(connectionString))
                {
                    ...
                    using (var sqlTx = db.BeginTransaction())
                    {
                        try
                        {
                            // Перевіряємо гаманець-відправник та його баланс і валюту
                            string getSourceSql = "SELECT WalletName, Currency, Balance FROM Wallets WHERE UserId = @UserId AND WalletName = @WalletName";
                            var sourceWallet = await db.QueryFirstOrDefaultAsync<Wallet>(getSourceSql, new { UserId = userId, WalletName = transfer.FromWallet }, transaction: sqlTx);

                            // Перевіряємо гаманець-відправник
                            if (sourceWallet == null)
                            {
                                sqlTx.Rollback();
                                await SendPacketAsync(stream, new MessagePacket { Type = MessageType.Error, Data = "Помилка: Гаманець-відправник не знайдено!" });
                                return;
                            }
                            ...
                            {
                                sqlTx.Rollback();
                                await SendPacketAsync(stream, new MessagePacket { Type = MessageType.Error, Data = "Помилка: Недостатньо коштів на рахунку!" });
                                return;
                            }

                            // Перевіряємо гаманець-отримувач
                            var targetWallet = await db.QueryFirstOrDefaultAsync<Wallet>(getSourceSql, new { UserId = userId, WalletName = transfer.ToWallet }, transaction: sqlTx);

                            if (targetWallet == null)
                            {
                                sqlTx.Rollback();
                                await SendPacketAsync(stream, new MessagePacket { Type = MessageType.Error, Data = "Помилка: Гаманець-отримувач не знайдено!" });
                                return;
                            }

                            // Розрахунок конвертації валют
                            decimal rate = GetRate(sourceWallet.Currency, targetWallet.Currency);
                            decimal convertedAmount = Math.Round(transfer.Amount * rate, 2);

                            // Оновлюємо баланси в базі даних
                            string updateSourceSql = "UPDATE Wallets SET Balance = Balance - @Amount WHERE UserId = @UserId AND WalletName = @WalletName";
                            await db.ExecuteAsync(updateSourceSql, new { Amount = transfer.Amount, UserId = userId, WalletName = transfer.FromWallet }, transaction: sqlTx);

                            string updateTargetSql = "UPDATE Wallets SET Balance = Balance + @Amount WHERE UserId = @UserId AND WalletName = @WalletName";
                            await db.ExecuteAsync(updateTargetSql, new { Amount = convertedAmount, UserId = userId, WalletName = transfer.ToWallet }, transaction: sqlTx);

                            //Логуємо дві транзакції в таблицю Transactions
                            string insertTxSql = @"INSERT INTO Transactions (UserId, WalletName, Amount, Category, IsIncome, Date) 
                                           VALUES (@UserId, @WalletName, @Amount, @Category, @IsIncome, @Date);";

                            // Транзакція списання
                            await db.ExecuteAsync(insertTxSql, new
                            {
                                UserId = userId,
                                WalletName = transfer.FromWallet,
                                Amount = transfer.Amount,
                                Category = $"Переказ на {transfer.ToWallet}",
                                IsIncome = false,
                                Date = DateTime.Now
                            }, transaction: sqlTx);

                            // Транзакція зарахування
                            await db.ExecuteAsync(insertTxSql, new
                            {
                                UserId = userId,
                                WalletName = transfer.ToWallet,
                                Amount = convertedAmount,
                                Category = $"Переказ з {transfer.FromWallet}",
                                IsIncome = true,
                                Date = DateTime.Now
                            }, transaction: sqlTx);
                            ...
                            // Відправляємо успішну відповідь разом із деталями для клієнта
                            string successMessage = $"Успішний переказ\n{transfer.Amount} {sourceWallet.Currency}\n→\n{convertedAmount} {targetWallet.Currency}";
                            await SendPacketAsync(stream, new MessagePacket { Type = MessageType.TransferResponse, Data = successMessage });
                        }
                        catch (Exception)
                        {
                            try { sqlTx.Rollback(); } catch { }
                            throw;
                        }
                    }
                }
            ...
            {
                Console.WriteLine($"[ERROR] Помилка переказу: {ex.Message}");
                await SendPacketAsync(stream, new MessagePacket { Type = MessageType.Error, Data = "Критична помилка на сервері під час переказу." });
            }
        }

        // Перенеси сюди свою логіку розрахунку курсів валют (або залиш базову 1 до 1, якщо валюта однакова)
        private static decimal GetRate(string fromCurrency, string toCurrency)
        {
            if (fromCurrency == toCurrency) return 1m;
            // Твоя логіка хардкоду курсів:
            if (fromCurrency == "USD" && toCurrency == "UAH") return 41.5m;
            if (fromCurrency == "UAH" && toCurrency == "USD") return 1 / 41.5m;
            return 1m;
        }
